...
import { Filter as FilterIcon, ChevronDown, Plus, Search, MoreHorizontal, Trash2, TrendingUp, TrendingDown, Check, X, Wallet, Clock, Globe, Pencil } from 'lucide-react';
...
// Live Clock Component for real-time engagement
...
// High-performance numeric animation engine
...
const TradeLog: React.FC<TradeLogProps> = ({ 
  ...
  }, [trades, searchTerm, statusFilter, selectedMonth]);
  ...
  const stats = useMemo(() => {
    ...
    const efficiency = ((filteredTrades.filter(t => t.status !== 'LOSS').length) / total) * 100;

    return { netPL, winRate, profitFactor, efficiency };
  }, [filteredTrades]);

  return (
    <div className="flex flex-col h-full p-4 pl-0">
      <header className="px-8 py-6 glass-panel rounded-[32px] mb-4 flex items-center justify-between relative z-50">
        <div className="flex items-center gap-10">
          {/* Account Switcher Component */}
          <div className="relative">
            <button 
              onClick={() => setIsAccountMenuOpen(!isAccountMenuOpen)}
              className="flex items-center gap-4 group text-left transition-all p-1 pr-4 rounded-3xl hover:bg-white/5 active:scale-95"
            >
            ...
            {isAccountMenuOpen && (
              <div className="absolute top-full left-0 mt-4 w-80 glass-panel rounded-[28px] p-2 border border-white/10 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300 backdrop-blur-3xl z-50">
                <div className="px-4 py-3 mb-2 flex justify-between items-center">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Select Managed Account</span>
                </div>
                  ...
                  {accounts.map((acc) => (
                    <div key={acc.id} className="group/acc relative flex items-center gap-1">
                      <button
                        ...
                        className={`flex-1 text-left px-4 py-3 rounded-2xl transition-all flex items-center justify-between group ${
                          currentAccount.id === acc.id ? 'bg-cyan-500/10 border border-cyan-500/20' : 'hover:bg-white/5 border border-transparent'
                        }`}
                      >
                        <div className="flex flex-col">
                      ...
                      {/* Action buttons for account (Rename/Delete) */}
                ...
                <div className="mt-2 pt-2 border-t border-white/5">
                  <button 
                    ...
                    className="w-full px-4 py-3 rounded-2xl text-[10px] font-black text-cyan-400 uppercase tracking-widest flex items-center gap-2 hover:bg-white/5 transition-all"
                  >
                    <Plus size={14} /> Add Funded Account
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              <button 
                onClick={() => { setIsFilterOpen(!isFilterOpen); setIsMonthOpen(false); }}
                ...
                <div className="absolute top-full left-0 mt-2 w-48 glass-panel rounded-2xl p-2 border border-white/10 shadow-2xl animate-in fade-in slide-in-from-top-2 duration-200">
                  {['ALL', 'WIN', 'LOSS', 'BE'].map((s) => (
                    <button
                      ...
                      className="w-full text-left px-4 py-2.5 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-white/10 transition-colors flex items-center justify-between"
                    >
            ...
            <div className="relative">
              <button 
                onClick={() => { setIsMonthOpen(!isMonthOpen); setIsFilterOpen(false); }}
                className={`flex items-center gap-2 text-xs font-bold px-4 py-2 rounded-xl border transition-all ${
                  selectedMonth !== 'ALL' 
              ...
              {isMonthOpen && (
                <div className="absolute top-full left-0 mt-2 w-48 glass-panel rounded-2xl p-2 border border-white/10 shadow-2xl max-h-64 overflow-auto no-scrollbar animate-in fade-in slide-in-from-top-2 duration-200">
                  <button
                    ...
                    className="w-full text-left px-4 py-2.5 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-white/10 transition-colors flex items-center justify-between text-slate-300"
                  >
                    All Months
                    {selectedMonth === 'ALL' && <Check size={14} className="text-cyan-400" />}
                  </button>
                  {months.map((m, idx) => (
                    <button
                      key={m}
                      onClick={() => { setSelectedMonth(idx); setIsMonthOpen(false); }}
                      className="w-full text-left px-4 py-2.5 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-white/10 transition-colors flex items-center justify-between text-slate-300"
                    >
                      {m}
                      {selectedMonth === idx && <Check size={14} className="text-cyan-400" />}
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="flex items-center gap-4">
            <div className="relative w-64 mr-2">
              ...
              <input 
                ...
                className="w-full pl-12 pr-10 py-2.5 text-sm bg-white/5 border border-white/10 rounded-xl focus:ring-2 focus:ring-cyan-500/50 focus:bg-white/10 outline-none transition-all text-white placeholder:text-slate-600 font-bold"
              />
              {searchTerm && (
            ...
            <button 
              onClick={onNewTrade}
              className="bg-gradient-to-r from-cyan-400 to-emerald-400 text-black px-6 py-2.5 rounded-xl font-black text-sm hover:opacity-90 transition-all shadow-lg glow-cyan flex items-center gap-2"
            >
      ...
      <div className="grid grid-cols-4 gap-4 mb-4">
        <StatCard 
          title="Net P&L" 
          numericValue={stats.netPL}
          trend={stats.netPL >= 0 ? 1 : -1}
          prefix="$"
          isPositive={stats.netPL >= 0}
        />
        <StatCard 
          title="Win Rate" 
          numericValue={stats.winRate} 
          trend={stats.winRate > 50 ? 1 : -1}
          suffix="%"
          decimals={1}
        />
        <StatCard 
          title="Profit Factor" 
          numericValue={stats.profitFactor} 
          trend={stats.profitFactor > 1.5 ? 1 : -1}
          decimals={2}
        />
        <StatCard 
          title="Efficiency" 
          numericValue={stats.efficiency} 
          trend={stats.efficiency > 70 ? 1 : -1}
          suffix="%"
          decimals={0}
        />
      </div>
                ...
                <tr>
                  <td colSpan={8} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-sm font-black text-slate-500 uppercase tracking-widest">No executions recorded yet</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filteredTrades.map((trade) => (
                  <tr key={trade.id} className="hover:bg-white/5 cursor-pointer transition-colors group">
                    ...
                    <td onClick={() => onTradeClick(trade)} className="px-8 py-6 text-center">
                      <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                        trade.status === 'WIN' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 
                        trade.status === 'LOSS' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : 'bg-white/10 text-slate-400 border border-white/10'
                      }`}>
                        {trade.status}
                      </span>
                    </td>
                    <td onClick={() => onTradeClick(trade)} className="px-8 py-6">
                      <span className={`text-sm font-black ${trade.side === 'LONG' ? 'text-cyan-400' : 'text-rose-400'}`}>{trade.side}</span>
                    </td>
                    <td onClick={() => onTradeClick(trade)} className={`px-8 py-6 text-right text-base font-black ${trade.netPL >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    ...
                    <td onClick={() => onTradeClick(trade)} className="px-8 py-6">
                      <span className="bg-indigo-500/10 text-indigo-300 text-[10px] font-black px-3 py-1.5 rounded-lg border border-indigo-500/20">{trade.setup}</span>
                    </td>
...
const StatCard: React.FC<{ 
  ...
  <div className="glass-panel p-6 rounded-[32px] flex flex-col gap-1 group hover:bg-white/10 transition-all border-white/10 overflow-hidden relative">
    ...
    <div className="flex items-end justify-between z-10">
      <AnimatedValue 
        ...
        suffix={suffix} 
        decimals={decimals}
        className={`text-2xl font-black transition-colors ${title === 'Net P&L' ? (isPositive ? 'text-emerald-400' : 'text-rose-400') : 'text-white'}`}
      />
